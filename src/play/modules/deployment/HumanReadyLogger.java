...
public class HumanReadyLogger implements MessageLogger, TransferListener {
  ...
  private Logger _logger = Logger.getLogger("play.modules.deployment.HumanReadyLogger");
  ...
  public void niceLog(String msg, int level) {
    try {
      ...
      if (msg.startsWith("found ")) { // Depedency found
        ...
        if (m.matches()) {
          _logger.info("~ \t" + m.group(1) + "->" + m.group(2) + " " + m.group(3) + " (from " + m.group(4) + ")");
          return;
        }
      }
      ...
      if (msg.startsWith("downloading ")) {
        if (downloading == null) {
          _logger.info("~");
          _logger.info("~ Downloading required dependencies,");
          _logger.info("~");
        }
        downloading = msg;
        progress = 0;
      ...
      if (msg.startsWith("[SUCCESSFUL")) {
        ...
        _logger.info("\r~ \t"
            + (downloading + "                    ").replace("(jar)", "").replace("downloading", "downloaded").replace("...", "   "));
      }
  ...
  public void transferProgress(TransferEvent te) {
    ...
    if (downloading != null) {
      ...
      _logger.info("\r~ \t"
          ...
          + " "
          + String.format("%-20s",
              FileUtils.byteCountToDisplaySize(length) + (te.isTotalLengthSet() ? "/" + FileUtils.byteCountToDisplaySize(te.getTotalLength()) : ""))
          + "\r");
